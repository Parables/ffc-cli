...
typed_cli_1.cli.commands(
  {
    program: "ffc",
    description:
      "Generates Freezed classes for Flutter from your GraphQL Schema",
    completer: true,
  },
  {
    ...
    generate: (0, typed_cli_1.command)({
      ...
      options: {
        schema: typed_cli_1.option.string
          .alias("s")
          .description(
            `the path to the GraphQL Schema. Default: ${code(
              "./schema.graphql"
            )}`
          )
          .validate(
            `Expected a GraphQL endpoint URL or valid GraphQL schema file with ${code(
              ".graphql"
            )} ${code(".json")} extension`,
            (value) =>
              value == undefined ||
              validator_1.default.isURL(value) ||
        ...
        output: typed_cli_1.option.string
          .alias("o")
          .description(
            `the full path of the output file. Default: ${code(
              "./lib/generated/app_models.dart"
            )}`
          ),
        ...
        fromJsonToJson: typed_cli_1.option.boolean
          ...
          .default(true)
          .description(
            `generate fromJson & toJson methods on the classes with json_serialization`
          ),
        lowercaseEnums: typed_cli_1.option.boolean
          .alias("e")
          .alias("enum")
          .default(true)
        ...
        mergeInputs: typed_cli_1.option.string
          ...
          .alias("merge")
          .description(
            `merge InputTypes of a pattern with an ObjectType as a union/sealed class`
          ),
        customScalars: typed_cli_1.option.string
          .alias("c")
          .alias("scalars")
          .description(
            `a JSON.stringify object of custom Scalars to Dart built-in types`
          ),
        watch: typed_cli_1.option.boolean
          ...
          .default(false)
          .description(
            `regenerate when GraphQL schemas changes. Accepts a boolean or an array of glob patterns`
          ),
        errorsOnly: typed_cli_1.option.boolean
          .alias("E")
          .default(false)
          .description(`print only errors`),
        usePolling: typed_cli_1.option.boolean
          .alias("p")
          .default(false)
          .description(`poll for changes`),
        interval: typed_cli_1.option.number
          .alias("T")
          .description(`poll every x millisecond`),
        overwrite: typed_cli_1.option.boolean
          .alias("O")
          .default(true)
          .description(`overwrite files if they already exist`),
        silent: typed_cli_1.option.boolean
          .alias("S")
          .default(false)
          .description(`suppress printing errors when they occur`),
      },
    }).handle((data) =>
      __awaiter(void 0, void 0, void 0, function* () {
        ...
        const _defaultFileName = _fileName.endsWith(".dart")
          ? _fileName.replace(".dart", "")
          : _fileName;
  ...
  if (!fs_1.default.existsSync("pubspec.yaml")) {
    console.warn(
      `${chalk_1.default.bold.yellow(
        "WARNING:"
      )} You are running from a non Flutter/Dart project directory\n`
    );
  }
}
function setSchema(schema) {
  ...
  if (fs_1.default.existsSync("./schema.json")) {
    ...
    console.error(
      ...
      )} Expected a GraphQL schema at the root of project folder or specify the path/URL with the ${chalk_1.default.italic.grey(
        "-s"
      )} or ${chalk_1.default.italic.grey("--schema")} option`
    );
    process.exit();
  }
}
